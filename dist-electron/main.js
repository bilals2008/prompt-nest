...
async function v() {
	let e = m(), t = await e.get("SELECT COUNT(*) as count FROM prompts WHERE is_template = 0"), n = await e.get("SELECT COUNT(*) as count FROM collections"), r = await e.get("SELECT COUNT(*) as count FROM prompts WHERE is_template = 1"), i = await e.get("SELECT COUNT(*) as count FROM prompts WHERE is_template = 0 AND created_at >= datetime('now', '-7 days')"), a = await e.get("SELECT COUNT(*) as count FROM prompts WHERE is_template = 0 AND created_at >= datetime('now', '-14 days') AND created_at < datetime('now', '-7 days')");
	return await e.get("SELECT COUNT(*) as count FROM prompts WHERE is_template = 0 AND created_at >= datetime('now', '-14 days') AND created_at < datetime('now', '-7 days')"), {
		totalPrompts: t?.count || 0,
		collections: n?.count || 0,
		totalTemplates: r?.count || 0,
		thisWeek: i?.count || 0,
		prevWeekPrompts: a?.count || 0
	};
}
async function y() {
	return (await m().get("SELECT COUNT(*) as count FROM activity"))?.count || 0;
}
async function b() {
...
async function x({ title: e, content: t, tags: n = "", notes: r = "", collection_id: i = null }) {
...
async function ee(e, { title: t, content: n, tags: r, notes: i, collection_id: a }) {
	let o = m(), s = (/* @__PURE__ */ new Date()).toISOString(), c = [], l = [];
	return t !== void 0 && (c.push("title = ?"), l.push(t)), n !== void 0 && (c.push("content = ?"), l.push(n)), r !== void 0 && (c.push("tags = ?"), l.push(r)), i !== void 0 && (c.push("notes = ?"), l.push(i)), a !== void 0 && (c.push("collection_id = ?"), l.push(a)), c.push("updated_at = ?"), l.push(s), l.push(e), await o.run(`UPDATE prompts SET ${c.join(", ")} WHERE id = ?`, l), S(e);
}
async function T(e) {
...
function J() {
	B.autoDownload = !1, B.autoInstallOnAppQuit = !1, B.on("checking-for-update", () => {
		q("checking-for-update");
	}), B.on("update-available", (e) => {
		let t = e.files?.some((e) => e.blockMapSize > 0);
		q("update-available", {
			version: e.version,
			releaseDate: e.releaseDate,
			releaseNotes: e.releaseNotes,
			updateType: t ? "patch" : "full"
		});
		...
		q("error", { message: e?.message || "Update error" });
	});
}
function Y() {
	G = new e({
...
function X() {
	K || J(), r.handle("db:createPrompt", (e, t) => x(t)), r.handle("db:getPromptById", (e, t) => S(t)), r.handle("db:getAllPrompts", () => C()), r.handle("db:updatePrompt", (e, t, n) => ee(t, n)), r.handle("db:deletePrompt", (e, t) => T(t)), r.handle("db:toggleFavorite", (e, t) => E(t)), r.handle("db:getFavorites", () => w()), r.handle("db:createCollection", (e, t) => j(t)), r.handle("db:getCollections", () => N()), r.handle("db:updateCollection", (e, t, n) => P(t, n)), r.handle("db:deleteCollection", (e, t) => F(t)), r.handle("db:logActivity", (e, t, n) => I(t, n)), r.handle("db:getActivity", (e, t) => L(t)), r.handle("db:exportData", (e, t) => R(t)), r.handle("db:importData", () => z()), r.handle("db:searchPrompts", (e, t, n) => A(t, n)), r.handle("db:getDashboardStats", () => v()), r.handle("db:getTemplates", () => D()), r.handle("db:createTemplate", (e, t) => O(t)), r.handle("db:deleteTemplate", (e, t) => k(t)), r.handle("db:getDatabaseStats", () => b()), r.handle("app:getVersion", () => t.getVersion()), r.handle("updater:get-app-version", () => {
		if (K) try {
		...
		if (K) return q("update-not-available"), {
			ok: !1,
			devMode: !0
		};
		try {
			return B.checkForUpdates(), { ok: !0 };
		} catch (e) {
			return q("error", { message: e?.message || "Update check failed" }), {
				ok: !1,
				message: e?.message
			};
		}
		...
		try {
			return B.downloadUpdate(), { ok: !0 };
		} catch (e) {
			return q("error", { message: e?.message || "Download failed" }), {
				ok: !1,
				message: e?.message
			};
		}
	...
	}), r.handle("app:getAutoStart", () => t.getLoginItemSettings().openAtLogin), r.handle("app:setAutoStart", (e, n) => (t.setLoginItemSettings({ openAtLogin: n }), n)), r.handle("db:openDbFolder", async () => {
		let e = o.join(t.getPath("userData"), "PromptNest");
		await i.openPath(e);
...
var Z = Date.now();
function Q(e) {
	let n = o.join(t.getPath("userData"), "PromptNest", "meta.json"), r = {};
	try {
		r = JSON.parse(s.readFileSync(n, "utf8"));
	} catch {}
	r.lastBackup = e, s.writeFileSync(n, JSON.stringify(r));
}
var $ = 0;
function te() {
	let e = o.join(t.getPath("userData"), "PromptNest", "meta.json"), n = {};
	try {
		n = JSON.parse(s.readFileSync(e, "utf8"));
	} catch {}
	$ = (n.sessionCount || 0) + 1, n.sessionCount = $, s.writeFileSync(e, JSON.stringify(n));
}
function ne() {
	let e = o.join(t.getPath("userData"), "PromptNest", "meta.json");
	try {
...
function re() {
	r.handle("app:getVersions", () => ({
		electron: process.versions.electron,
		chrome: process.versions.chrome,
		node: process.versions.node,
		v8: process.versions.v8
	})), r.handle("app:getUptime", () => Date.now() - Z), r.handle("app:getSessionCount", () => $), r.handle("app:getLastBackup", () => ne().lastBackup || null), r.handle("app:getTotalActivity", async () => {
		try {
			return await y();
		} catch {
			return 0;
		}
	}), r.handle("app:getDiskFree", async () => {
		try {
			let e = t.getPath("userData"), { execSync: n } = await import("node:child_process"), r = n(`powershell -Command "(Get-PSDrive -Name $((Get-Item '${e.replace(/'/g, "''")}').PSDrive.Name)).Free"`, {
				encoding: "utf8",
				timeout: 3e3
			}).trim();
